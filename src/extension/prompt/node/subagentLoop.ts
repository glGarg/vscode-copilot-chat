...
export class SubagentToolCallingLoop extends ToolCallingLoop<ISubagentToolCallingLoopOptions> {
				...
				if (result instanceof LanguageModelToolResult2) {
					...
					result.content.forEach((part, idx) => {
						if (part instanceof LanguageModelTextPart) {
							...
							const partName = (part as { constructor?: { name?: string } } | null)?.constructor?.name ?? 'Unknown';
							this._logService.info(`[SubagentToolCallingLoop]   Part ${idx + 1}: ${partName}`);
						}
					});
				} else {
					this._logService.info(`[SubagentToolCallingLoop]   Result type: ${typeof result}`);
					this._logService.info(`[SubagentToolCallingLoop]   Result: ${JSON.stringify(result)}`);
				}
			}
			this._logService.info('[SubagentToolCallingLoop] ========================================');
		}
	...
	private async logAvailableModels() {
		try {
			...
			this._logService.warn(`[SubagentToolCallingLoop] Failed to list available models: ${error instanceof Error ? error.message : String(error)}`);
		}
	}
	...
	private async getEndpoint(request: ChatRequest) {
		...
		try {
			...
			this._logService.info(`[SubagentToolCallingLoop] Successfully selected endpoint: ${JSON.stringify({
